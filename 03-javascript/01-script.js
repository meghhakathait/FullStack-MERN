...
// the range of num is (2^53-1) to -(2^53-1) and the range of the bigint is more than that. to make the data type of a number to bigint we can add n at the end of the number
...
// null is special data type that represents the absence of any value . it is often used to indicate that a varibale has no value or that an object property is empty . the typeof null returns "object" which is a quirk in javascript and it is not a Bug. it is a result of the way javascript was designed and it has been kept for backword compatibility


// let username;
// we can intialse nd assign value later while using let but not with const
